...
$dad_cad = mysql_query ("SELECT *,m.obs AS mobs, DATE_FORMAT(m.datanasc,'%d/%m/%Y') AS br_datanasc, m.datanasc AS nasc, DATE_FORMAT(m.datanasc,'%d') AS dia FROM membro AS m, eclesiastico AS e WHERE m.rol='".$_SESSION["rol"]."' AND m.rol=e.rol");
...
?>
	...
	<table width="580">
      <tr>
		...
		?><div><img src="img_membros/<?PHP echo "$img";?>" width="75" height="98" border="1" /></div></td>
      </tr>
      <tr>
        <td >Pai:
        <?PHP
