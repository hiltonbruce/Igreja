<h3><a href="./?escolha=adm/dados_disciplina.php&bsc_rol=<?php echo $bsc_rol;?>&novo=novo">Novo Registro, Clique aqui! ...</a></h3>
...
if ($_GET["lista"]<1 && empty ($_GET["novo"]) && empty($_POST["Submit"]) ) {
	...
	$paginas = ceil ($total/$nmpp); //Retorna o total de páginas
			...
			<?PHP
				...
				$Tipo = new situacao_espiritual ($coluna['motivo'],$_GET['rol']);
				$ls+=1;
				if ($ls>1)	
						{
						$cor="class='odd'";
				...
				<tr <?php echo "$cor";?>>
					...
					<td><a href="./?escolha=adm/dados_disciplina.php&lista=<?php echo $coluna["id"];?>" 
							title="Click aqui para o texto completo!"><?php echo substr($coluna["motivo"],0,35)."...";?>
						</a></td>
...
if (!empty($_GET["lista"])) {
	...
	<table border="1"  width="100%">
      <tr>
            ...
            <td><p>Tipo de Registro:</p>
			<?PHP echo $Tipo->situacao_confirma();?>
			</td>
      </tr>
          <tr>
            <td colspan="3" bgcolor="#E9E9E9"><p>Descri&ccedil;&atilde;o:</p>
              <?PHP echo $disc_completa->motivo();?></td>
      </tr>
          <tr>
            <td><p>In&iacute;cio:</p>
              <?PHP echo conv_valor_br ($disc_completa->data_ini());?>
			</td>
            <td colspan="2"><p>Final:</p><?PHP echo conv_valor_br ($disc_completa->data_fim());?></td>
          </tr>
</table>
	<?PHP
...
?>
	...
	<form id="form1" name="form1" method="post" action="">
	  ...
	  <thead>
        <tr>
          <td width="327" rowspan="5" id="form"><label>Detalhamento:
              <textarea name="motivo" cols="30" rows="6" required="required" id="motivo" tabindex="<?php echo ++$ind;?>"></textarea>
          </label
		  ></td>
          <td>Dirigente do Culto: 
            <input name="dirigente" type="text"  tabindex="<?php echo ++$ind;?>" /></td>
        </tr>
        <tr>
          <td>		</td>
		</tr>
	    <tr>
		<td><label>Situacao Espiritual</label>
		<select name="situacao"  tabindex="<?PHP echo ++$ind;?>">
          ...
          <tr>
            <td width="229">Data do Registro:
        <label><input type="text" name="data_ini" id='data' tabindex="<?php echo ++$ind;?>"/></label>(Em branco para data atual)	  </td></tr>
 
        <tr>
          <td>Tempo em dias (para disciplina):
            <label><input type="text" name="prazo" tabindex="<?php echo ++$ind;?>" /></label>
(Em branco para INDETERMINDO)</td>
        </tr>
		</thead>
      </table>
...
?>
	...
	<form id="form1" name="form1" method="post" action="">
		Descri&ccedil;&atilde;o: <?php echo $_POST["motivo"]."<h5>Culto dirigido por: ".$_POST[dirigente]."</h5>";?>
		<label>
			<input type="hidden" name="motivo" value="<?php echo $_POST["motivo"]."<h5>Culto dirigido por: {$_POST[dirigente]}</h5>";?>">
		</label>
		Registro: <?PHP echo $situacao->situacao_confirma();?>
		<label><input type="hidden" name="situacao" value="<?PHP echo $_POST["situacao"];?>"></label>
		<label>Data:
		  <?php 
		  if ($_POST["data_ini"]<>"") {
		  		echo $_POST["data_ini"];
		  ...
		  ?>
            <input type="hidden" name="data_ini" value="<?php echo $_POST["data_ini"];?>"/>
		</label>
	<table  width="100%">
	  <thead>
        <tr>
	  ...
	  <label>
	  <input type="submit" name="Submit" value="Confimar..." tabindex="<?php echo ++$ind;?>" />
	  </label>
	</form>
<?PHP
