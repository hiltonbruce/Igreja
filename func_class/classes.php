	...
	class rodape {
		...
		public function getRodapeinic() {
			if ($this->_url>1)
			{
				...
				echo 	"<a href='./{$this->getUrlextra()}&{$this->_linkpagina}=1'>In&iacute;cio</a> | ".
						"<a href='./{$this->getUrlextra()}&{$this->_linkpagina}=$url'>Anterior</a>"; //Vai p a p�gina anterior
			}
		}
		...
		*/
		public function getRodapemeio() {
			//Define a parte central da pagina��o
...
class insert {
	...
	public function inserir() {
		...
		if ($inserir){
				$idCad =  "<script> alert('Inclus�o ######$idCad### realizada com sucesso! Em ".$this->getTabela()."');</script>";
...
class sele_cidade {
	...
	function ListDados ($indice){//indice da sequ�ncia do formul�rio
