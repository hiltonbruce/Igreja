...
class sele_cidade {
	...
	function ListDados ($indice,$id=null,$pers=null){//indice da sequï¿½ncia do formulï¿½rio
	global $db;
...
// 					echo "<option value=''>-->> Escolha  <<--</option>";
	...
	function cidadePopup ($indice,$id=null,$link=null){//indice da sequï¿½ncia do formulï¿½rio
	global $db;
	$sql_lst = "SELECT * from {$this->tabela} WHERE {$this->campo}=? ORDER BY {$this->campo_retorno}";
	$this->res = $db->query($sql_lst, array( $this->valor ));
	//Obtï¿½m o nï¿½mero de linhas
...
// 					echo "<option value=''>-->> Escolha  <<--</option>";
	...
	//$db->disconnect();
		...
		echo "Voc&ecirc; n&atilde;o informou o estado de batismo! Fa&ccedil;a-o antes de continuar.</h2>";
		echo "<script> alert('Vocï¿½ não informou o estado de batismo! Faï¿½a-o antes de continuar.'); window.history.go(-1);</script>";
		exit;
	}elseif ($this->campo==strtolower("idcidade") && $this->valor == ""){
		echo "Vocï¿½ não informou a cidade ou falta atualizar! Faï¿½a-o antes de continuar.</h2>";
		echo "<script> alert('Vocï¿½ não informou a cidade ou falta atualizar! Faï¿½a-o antes de continuar.'); window.history.go(-1);</script>";
	}else{
