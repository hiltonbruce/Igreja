...
function calcularDiaSemana($dia,$mes,$ano) {
 ...
 function gerarCalendario($mes,$ano,$nmeses,$ncols,$datas,$rodapes,$leg,$dia_ceia,$semana_ceia)//$feriados,$marcados,$rodapes)
 {
  ...
  {
   $tabela="Erro ao gerar calendário: [m&ecirc;s=".$mes."] [ano=".$ano.
   ...
   $dia=$daux=$dia_semana;
    for($i=0;$i<count($datas);$i++)
      $qtd[$i]=count($datas[$i]);
     $nq=count($qtd);
     $tabela="<table class='table'>"; //Inicia a tabela geral (que suportará as demais tabelas de meses)
     while($idx<$total)
     {
      $tabela .= "<tr>";
      for($ms=0; $ms<$ncols && $idx<$total; $ms++)
      {
       $temp_tb="<td valign='top'><table class='table table-bordered'>
                ...
                "</h4></td></tr><tr>"; //Cria uma tabela para o mês atual
