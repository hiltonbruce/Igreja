...
class sec_membro {
	...
	function __construct ($rolMembro='') {
		...
		$sqlConsulta .= 'p.cpf,p.rg,p.obs AS obsProf,p.orgao_expedidor,p.profissao,p.onde_trabalha, ';
		$sqlConsulta .= 'e.*, c.rol_conjugue,c.conjugue,c.estado_civil ';
		$sqlConsulta .= 'FROM eclesiastico AS e,igreja AS i,membro AS m, est_civil AS c ';
		$sqlConsulta .= ',profissional AS p ';
		$sqlConsulta .= 'WHERE e.congregacao=i.rol AND m.rol=c.rol AND p.rol=c.rol AND m.rol=e.rol AND ';
	...
	function dadosMembro () {
		...
		{
				$arrayMembro = array('rol'=>$dados['rol'],'nome'=>$dados['nome'],'bairro'=>$dados['bairro']
				,'celular'=>$dados['celular'],'cep'=>$dados['cep'],'cidade'=>$dados['cidade']
						...
						,'uf_resid'=>$dados['uf_resid'],'auxiliar'=>$dados['auxiliar'],'batismo_em_aguas'=>$dados['batismo_em_aguas'],'local_batismo'=>$dados['local_batismo']
						...
						,'ufEcle'=>$dados['ufEcle'],'conjugue'=>$dados['conjugue'],'dataCivil'=>$dados['dataCivil'],'estado_civil'=>$dados['estado_civil']
						,'rol_conjugue'=>$dados['rol_conjugue'],'obsCivil'=>$dados['obsCivil']
