...
$queryLanc .= ' WHERE DATE_FORMAT(l.data,"%Y%m") = "201412" AND';
...
while ($contas = mysql_fetch_array($lista)) {
   ...
   $dados = new insert ($value,'lanc2');
   $dados->inserir();
   //$idAtual = mysql_insert_id();
   //$Atualizacao = mysql_query('UPDATE lanc2 SET debitar = "'.$contas['conta'].'" WHERE valor = "'.$contas['valor'].'" AND lancamento="'.$contas['lancamento'].'"');
   /*print_r($contas);*/
}
