...
class bairro {
	...
	function Arraybairro($idcidade){
		$query = "SELECT id,bairro FROM bairro WHERE idcidade=$idcidade ";
		$sqlBairro = mysql_query("$query ORDER BY bairro") or die (mysql_error());
	//echo "<h1>Teste</h1>";
	       while($lst = mysql_fetch_array($sqlBairro))
	       {
		    $bairr_array [$lst['id']] = $lst['bairro'];
	       }
	  return $bairr_array;
	}
