...
class agenda {
	...
	function nomes () {
		...
		{
			$mud_acent = strtoupper(strtr($this->dados["nome"], '·‡„‚ÈÍÌÛıÙ˙¸Á¡¿√¬… Õ”’‘⁄‹«','AAAAEEIOOOUUCAAAAEEIOOOUUC' ));
			$todos[$ind++] = $mud_acent;
		}
		return $todos ;
	}
	...
	function insdespmes(){
		...
		//verifica o m√™s da ultima conta e inseri as atuais e as que estiverem com
		//vencimento at√© o dia 10 de cada m√™s ser√° inserida j√° para o pr√≥ximo m√™s
		...
		while ($contas = mysql_fetch_array($lista)) {
			...
			//echo '<br/>dia: '.$dia.' - M√™s: '.$mes.' - Ano: '.$ano;
			...
			if (ceil( (mktime() - mktime(0,0,0,$mesv,$diav,$anov))/(3600*24))>'0' && $id['status']<>'3') {
				...
				for ($i = 1; $i <= $mesvenc; $i++) {
					...
					$value	.='","'.$id['resppgto'].'","0000-00-00","0","'.date('d/m/Y H:i:s').', '.$_SESSION['valid_user'] .', Registro autom·tico"';
	...
	while($coluna_fix = mysql_fetch_array($sql3_fix))
	{
		...
		switch ($coluna_fix['status']) {
			...
			case 2:
				...
				$evento .= 'alt="D&iacute;ÔøΩvida Paga! Obrigado."></span>'.$idLanc;
				$status = $evento.' Pago em: '.conv_valor_br ($coluna_fix['datapgto']);
				$status .= ' - '.$coluna_fix['resppgto'];
				$titulo = 'D&iacute;ÔøΩvida Paga! Obrigado.';
				break;
			case 3:
				$status = 'Quitado';
				$titulo = 'Quitado';
			...
			default:
				if (date("Y-m-d")==$coluna_fix['vencimento']) {
					...
					$titulo = 'D&iacute;ÔøΩvida n&atilde;o Paga!';
					...
					$titulo = 'D&iacute;ÔøΩvida Vencida!';
				}else {
					$status .= '<span class="glyphicon glyphicon-warning-sign text-warning"';
					$status .= ' aria-hidden="true" ></span>FaltaÔøΩ Pagar';
	...
	function motivo($motivo,$credor) { //Lista contas agendas por motivo
		...
		while ($contas = mysql_fetch_array($listvenc_array)) {
			if ($contas['status']=='2') {//Marca os j√° pagos
				...
				$evento = '<span class="glyphicon glyphicon-ok text-success"';
				$evento .= ' aria-hidden="true" ></span>  '.$idLanc;
				$titulo = 'D&acute;ÔøΩvida Paga! Obrigado. Motivo:'.$contas['motivo'];
			}elseif ($contas['status']=='1'){
				$evento  = '<span class="glyphicon glyphicon-warning-sign text-warning"';
				$evento .= ' aria-hidden="true" ></span> ';
				...
				$evento  = '<span class="glyphicon glyphicon-remove-sign text-danger"';
				$evento .= ' aria-hidden="true" ></span> ';
				$titulo = 'D&iacute;ÔøΩvida vencida! Motivo:'.$contas['motivo'];
			}else {
				$evento ='';
				$titulo = 'Click aqui atualizar!';
			}
	...
	function demonstrativo ($dia,$credor,$pagamento) {
		...
		while ($periodo_dados =mysql_fetch_array($periodo_array)) {
			if ($periodo_dados['status']=='2') {//Marca os j√° pagos
				...
				$evento = '<span class="glyphicon glyphicon-ok text-success"';
				$evento .= ' aria-hidden="true" ></span>  '.$idLanc;
				$titulo = 'D&iacute;ÔøΩvida Paga! Obrigado. Motivo: '.$periodo_dados['motivo'];
			}elseif ($periodo_dados['status']=='1'){
				$evento  = '<span class="glyphicon glyphicon-warning-sign text-warning"';
				$evento .= ' aria-hidden="true" ></span> ';
				...
				$evento  = '<span class="glyphicon glyphicon-remove-sign text-danger"';
				$evento .= ' aria-hidden="true" ></span> ';
				$titulo = 'D&iacute;ÔøΩvida vencida! Motivo: '.$periodo_dados['motivo'];
			}else {
				$evento ='';
				$titulo = 'Click aqui atualizar! Motivo: '.$periodo_dados['motivo'];
			}
